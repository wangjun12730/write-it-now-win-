...
class StoryShowController extends Controller
{
    ...
    public function readerStoryShow(){
        ...
        $comments = $data[$res['user_id']][$s_id]["书评"];

        //反转数组，使最新评论放在最前
        $comments = array_reverse($comments);

        $count_comments = count($comments); //统计该本书评论数
        for($i=0;$i< $count_comments;$i++){//获取评论者信息

            $like_count = count($comments[$i][2]); //统计点赞数
            $comments[$i]['like_count'] = $like_count; //将点赞数存进数据数组
            $info = M('members')->where('id='.$comments[$i][0])->find();
            if($info){
                $comments[$i]['user_info'] = $info;  //将评论者信息存入评论数据数组
            }

        }

        $this->assign('count_comments',$count_comments);
