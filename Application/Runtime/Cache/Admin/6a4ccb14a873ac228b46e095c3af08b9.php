...
<div class="check">
    ...
    <div  style="margin-bottom: 30px;margin-top: 20px" >
        <a class="btn btn-primary" href="/write-it-now-win-/Admin/Check/checkList">未审核</a><a class="btn btn-warning" style="margin-left: 10px">已审核</a>
    </div>
    <?php if($empty==1): ?><h3>现在还没有已审核的信息</h3>
    <?php else: ?>
        <table class="table table-striped table-hover table-condensed">
            ...
            <?php if(is_array($result)): foreach($result as $key=>$v): ?><tr>
                    ...
                    <td><div   data-container="body" data-toggle="popover" data-trigger="hover" data-placement="bottom" data-content="<?php echo ($v["content"]); ?>" title="审核内容" style="overflow: hidden; white-space: nowrap; text-overflow: ellipsis;width:145px;">
                        <?php echo ($v["content"]); ?>
                    </div></td>
                    <td><?php echo ($v["font_count"]); ?></td>
                    <td><?php echo ($v["is_doing"]); ?></td>
                    <td><?php echo ($v["time"]); ?></td>
                    <td><?php echo ($v["type"]); ?></td>
                    <td>
                       已审核
                    </td>
                    <td><a class="btn btn-primary" href="/write-it-now-win-/Admin/Check/deleteCheck/c_id/<?php echo ($v["id"]); ?>">删除</a></td>
