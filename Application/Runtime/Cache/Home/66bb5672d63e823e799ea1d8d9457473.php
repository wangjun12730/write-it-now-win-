...
<div class="container info">
    <h4 class="classify"><a href="/win/write-it-now-win-/Home/Index/index">首页</a> <span class="glyphicon glyphicon-menu-right"></span> <a href="/win/write-it-now-win-/Home/Tag/tag/type/<?php echo ($res["tag"]); ?>" target="_blank"><?php echo ($res["tag"]); ?>频道</a> <span class="glyphicon glyphicon-menu-right"></span> <a><?php echo ($res["name"]); ?></a></h4>
    <div class="story-info">
        <div class="row">
            <div class="col-sm-2 ">
                <div class="book-img">
                    <a>
                        <img src="/win/write-it-now-win-/Public/<?php echo ($res["picture"]); ?>" />
                    </a>
                </div>
            </div>
            <div class="col-sm-10">
                <div class="book-info">
                    ...
                    <p>
                        <?php if($is_follow == 0): ?><form action="/win/write-it-now-win-/Home/Collection/collection" id="collection" method="post" target="_blank" style="display: inline-block">
                            ...
                            <a class="btn btn-danger " href="/win/write-it-now-win-/Home/StoryShow/storyInfo/s_id/<?php echo ($res["id"]); ?>">
                                开始阅读
                            </a>
                         ...
                         <?php else: ?>
                            <form action="/win/write-it-now-win-/Home/Collection/uncollection" id="uncollection" method="post" >
                                <input type="hidden" value="<?php echo ($res["user_id"]); ?>" name="author_id"/>
                                <input type="hidden" value="<?php echo ($res["id"]); ?>" name="story_id"/>
                                <input type="hidden" value="<?php echo (session('user_id')); ?>" name="follower_id"/>
                            </form>
                        ...
                        <a href="/win/write-it-now-win-/Home/StoryShow/sectionList/s_id/<?php echo ($res["id"]); ?>" style="float: right;margin-top: 13px;color:#a6a6a6 "><span class="glyphicon glyphicon-list" style="margin-left: 100px;margin-top: 5px"></span> <span>全部目录</span></a>
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="update">
        <div class="row">
            ...
            <p>最新章节</p>
                <p><a href="/win/write-it-now-win-/Home/StoryShow/storyInfo/s_id/<?php echo ($res["id"]); ?>/section/<?php echo ($newsection["id"]); ?>" class="title"><?php echo ($newsection["section"]); ?>&nbsp; <?php echo ($newsection["title"]); ?></a></p>
            </div>
        </div>
    </div>
    <hr />
    <div class="recom">
        <a href="/win/write-it-now-win-/Home/StoryShow/readerStoryShow/s_id/<?php echo ($res["id"]); ?>" style="float: right;"><span class="glyphicon glyphicon-repeat" style="color: #a6a6a6"></span> 换一换</a>
        <h4>精彩推荐</h4>
        <hr />
        <div class="row">
            <?php if(is_array($recom)): foreach($recom as $key=>$v): ?><div class="col-sm-2">
                    <a href="/win/write-it-now-win-/Home/StoryShow/readerStoryShow/s_id/<?php echo ($v["id"]); ?>">
            ...
            <ul>
                <?php if(is_array($comments)): foreach($comments as $key=>$v): ?><li>
                        <div class="row" style="margin-left: 0">
                            <div class="col-sm-1">
                                <div class="user_head">
                                    <a href="/win/write-it-now-win-/Home/Index/showAuthor/author/<?php echo ($v["user_info"]["id"]); ?>"><img class="img-circle" src="/win/write-it-now-win-/Public/<?php echo ($v["user_info"]["picture"]); ?>"/></a>
                                </div>
                            </div>
                            <div class="col-sm-11" style="margin-left: -20px">
                                <a href="/win/write-it-now-win-/Home/Index/showAuthor/author/<?php echo ($v["user_info"]["id"]); ?>" class="reader-name"><b><?php echo ($v["user_info"]["name"]); ?></b></a>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="/win/write-it-now-win-/Home/Comments/book_comments" method="post" id="form1">
